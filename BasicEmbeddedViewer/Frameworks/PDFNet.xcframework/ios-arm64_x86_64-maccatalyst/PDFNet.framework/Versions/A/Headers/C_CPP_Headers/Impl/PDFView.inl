...
inline ColorPt PDFView::GetPostProcessedColor(const ColorPt& col) const
{
	ColorPt result;
	REX(TRN_PDFViewGetPostProcessedColor(mp_view, &col.m_c, &result.m_c));
	return result;
}
...
inline void PDFView::SetFieldHighlightColor(const ColorPt& new_field_highlight_color)
{
	REX(TRN_PDFViewSetFieldHighlightColor(mp_view, &new_field_highlight_color.m_c));
}
...
inline void PDFView::SetSignatureHighlightColor(const ColorPt& new_signature_highlight_color)
{
	REX(TRN_PDFViewSetSignatureHighlightColor(mp_view, &new_signature_highlight_color.m_c));
}
...
inline PDFView::TextSelectionMode PDFView::GetTextSelectionMode() const
{
    enum TRN_PDFViewTextSelectionMode tm;
	REX(TRN_PDFViewGetTextSelectionMode(mp_view, &tm));
	return (PDFView::TextSelectionMode)tm;
}
...
inline bool PDFView::GetRightToLeftLanguage() const
{
	TRN_Bool result = 0;
	REX(TRN_PDFViewGetRightToLeftLanguage(mp_view, &result));
	return TBToB(result);
}
...
inline std::vector<int> PDFView::GetVisiblePages() const
{
	...
	std::vector<int> result((std::vector<int>::size_type)c);
	for (int i = 0; i < c; i++)
	{
		REX( TRN_PDFViewGetVisiblePage(mp_view, i, &res ));
		result[(std::vector<int>::size_type)i] = res;
	}
	return result;
}
...
inline bool PDFView::HasChangesSinceSnapshot()
{
    RetBool(TRN_PDFViewHasChangesSinceSnapshot(mp_view, &result));
}
...
inline bool PDFView::CanUndo()
{
	RetBool(TRN_PDFViewCanUndo(mp_view, &result));
}

inline bool PDFView::CanRedo()
{
	RetBool(TRN_PDFViewCanRedo(mp_view, &result));
}
...
inline void PDFView::RevertChangesSinceSnapshot()
{
    REX(TRN_PDFViewRevertChangesSinceSnapshot(mp_view));
}
...
inline void PDFView::SetPageBox(Page::Box region)
{
	REX(TRN_PDFViewSetPageBox(mp_view,(enum TRN_PageBox)region));
}

inline Page::Box PDFView::GetPageBox() const
{
	enum TRN_PageBox result;
	REX(TRN_PDFViewGetPageBox(mp_view, &result));
