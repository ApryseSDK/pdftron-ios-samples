...
public:
	...
	*
	* @note With the exception of SetPersistentCache, SetTempPath, and SetAPIDataOfflineProc, it is unsafe to call any other PDFNet API without first initializing 
	* the library
	* 
	...
	* the library
	*
	* @param license_key A license key used to activate the product.
	* @param json_string A json string contains useful info such as language 
	*
	* @exception If the license_key is invalid, the function will throw an exception.
	*/
	static void Initialize(const char* license_key, const char* json_string);
#endif
...
//#endif
	...
	*/
	 ...
	 *
	 * @param ReadProc A pointer to a function that will parse a floating-point number
	...
	*/
	...
	{
		e_continue, // Apryse SDK will continue running and if necessary switch to demo mode when encountering an unresolvable connection error
		e_continue_unless_switching_to_demo, // Apryse SDK will continue as long as it doesn't need to switch to demo mode in response to the connection error
											// If continuing without switching to demo mode is not possible, Apryse SDK functions will throw exceptions.
		e_stop // Apryse SDK will throw exceptions from subsequent usage if a connection error occurs
	};
	...
	*
	* @param message The reason for the connection failure.
	* @param error_code The error code from the connection failure.
	* @param switch_to_demo Is 1 if continuing in the current situation requires a change from
	...
	*/
#ifdef SWIG
	static void SetConnectionErrorProc(PDF::Callback* instance);
#else
	 ...
	 * A type of callback function that is called periodically to report API usage.
	 * 
	 * @param api_data_json A JSON string containing the API usage data.
	 * @param user_data The custom data passed to SetAPIDataOfflineProc.
	 */
	typedef void(*APIDataOfflineProc) (const char* api_data_json, void* user_data);

	/**
	 * Sets the API usage data handling callback to report API usage offline.
	 * @note This function needs to be called before PDFNet::Initialize.
	 * 
	 * @param api_data_proc API usage data handling callback.
	 * @param user_data Custom data to be passed as the second parameter to 'api_data_proc'.
	 */
#ifdef SWIG
	static void SetAPIDataOfflineProc(PDF::Callback* instance);
#else
	static void SetAPIDataOfflineProc(APIDataOfflineProc api_data_proc, void* user_data);
#endif
