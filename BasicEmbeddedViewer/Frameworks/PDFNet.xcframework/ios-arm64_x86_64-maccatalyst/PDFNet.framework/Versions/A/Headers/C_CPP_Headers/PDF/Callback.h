...
// PDFView Callbacks
	...
	virtual void ConnectionErrorProc(const char* message, const int error_code, UChar switch_to_demo) {}

	static void StaticConnectionErrorProc(const char* message, const int error_code, UChar switch_to_demo, void* data)
	{
		Callback* cb = (Callback*)data;
		cb->ConnectionErrorProc(message, error_code, switch_to_demo);
	}

	virtual void APIDataOfflineProc(const char* api_data_json) {}

	static void StaticAPIDataOfflineProc(const char* api_data_json, void* user_data)
	{
		Callback* cb = (Callback*)user_data;
		cb->APIDataOfflineProc(api_data_json);
	}
