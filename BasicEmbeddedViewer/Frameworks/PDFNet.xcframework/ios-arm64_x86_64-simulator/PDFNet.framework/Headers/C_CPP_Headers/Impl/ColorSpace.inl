...
inline void ColorSpace::InitComponentRanges(std::vector<double>& out_decode_low, std::vector<double>& out_decode_range) {
	const int num_comps=GetComponentNum();
	out_decode_low.resize((std::vector<double>::size_type)num_comps);
	out_decode_range.resize((std::vector<double>::size_type)num_comps);
