...
#ifdef SWIG
inline void PDFDoc::SaveCustomSignature(const std::vector<UChar>& in_signature, const DigitalSignatureField& in_field, const UString& in_path)
{
	REX(TRN_PDFDocSaveCustomSignature(mp_doc, in_signature.empty() ? 0 : &in_signature.front(), in_signature.size(), in_field.m_impl, in_path.mp_impl));
}

inline void PDFDoc::SaveCustomSignature(const std::vector<UChar>& in_signature, const DigitalSignatureField& in_field, Filters::Filter& out_stream)
{
	REX(TRN_PDFDocSaveCustomSignatureStream(mp_doc, in_signature.empty() ? 0 : &in_signature.front(), in_signature.size(), in_field.m_impl, out_stream.m_impl));
}
#else
inline void PDFDoc::SaveCustomSignature(const UChar* in_signature, const size_t in_signature_size, const DigitalSignatureField& in_field, const UString& in_path)
{
	REX(TRN_PDFDocSaveCustomSignature(mp_doc, in_signature, in_signature_size, in_field.m_impl, in_path.mp_impl));
}

inline void PDFDoc::SaveCustomSignature(const UChar* in_signature, const size_t in_signature_size, const DigitalSignatureField& in_field, Filters::Filter& out_stream)
{
	REX(TRN_PDFDocSaveCustomSignatureStream(mp_doc, in_signature, in_signature_size, in_field.m_impl, out_stream.m_impl));
}

inline void PDFDoc::SaveCustomSignature(const UChar* in_signature, const size_t in_signature_size, const DigitalSignatureField& in_field, const UChar* &out_buf, size_t& out_buf_size)
{
	REX(TRN_PDFDocSaveCustomSignatureBuffer(mp_doc, in_signature, in_signature_size, in_field.m_impl, &out_buf, &out_buf_size));
}
#endif
...
{
	 REX(TRN_PDFDocInsertPages(mp_doc, (TRN_UInt32)insert_before_page_number, sourceDoc.mp_doc,
		 (TRN_UInt32)startPage, (TRN_UInt32)endPage, (enum TRN_PDFDocInsertFlag) flag));
}
...
{
	REX(TRN_PDFDocInsertPageSet(mp_doc, (TRN_UInt32)insert_before_page_number,
		sourceDoc.mp_doc, sourcePageSet.mp_impl, (enum TRN_PDFDocInsertFlag) flag));
}
...
{
	REX(TRN_PDFDocMovePages(mp_doc, (TRN_UInt32)moveBeforeThisPage, sourceDoc.mp_doc,
		(TRN_UInt32)startPage, (TRN_UInt32)endPage, (enum TRN_PDFDocInsertFlag) flag));
}
...
{	
	REX(TRN_PDFDocMovePageSet(mp_doc, (TRN_UInt32)moveBeforeThisPage,
		sourceDoc.mp_doc, sourcePageSet.mp_impl, (enum TRN_PDFDocInsertFlag) flag));
}
...
inline int PDFDoc::AppendTextDiff(Page page1, Page page2) {
	int result;
	REX(TRN_PDFDocAppendTextDiffPage(mp_doc, page1.mp_page, page2.mp_page, &result));
	return result;
}

inline int PDFDoc::AppendTextDiff(PDFDoc& doc1, PDFDoc& doc2) {
	int result;
	REX(TRN_PDFDocAppendTextDiffDoc(mp_doc, doc1.mp_doc, doc2.mp_doc, 0, &result));
	return result;
}

inline int PDFDoc::AppendTextDiff(PDFDoc& doc1, PDFDoc& doc2, TextDiffOptions* options) {
	int result;
	TRN_optionbase opt_in;
	opt_in.type = e_option_type_sdf;
	opt_in.impl = options ? options->GetInternalObj().mp_obj : 0;
	REX(TRN_PDFDocAppendTextDiffDoc(mp_doc, doc1.mp_doc, doc2.mp_doc, &opt_in, &result));
	return result;
}

inline int PDFDoc::HighlightTextDiff(PDFDoc& doc1, PDFDoc& doc2, TextDiffOptions* options) {
	int result;
	TRN_optionbase opt_in;
	opt_in.type = e_option_type_sdf;
	opt_in.impl = options ? options->GetInternalObj().mp_obj : 0;
	REX(TRN_PDFDocHighlightTextDiff(doc1.mp_doc, doc2.mp_doc, &opt_in, &result));
	return result;
}
...
inline std::vector<unsigned char> PDFDoc::SaveViewerOptimized(const ViewerOptimizedOptions& opts) {
	size_t size;
	char* buf;
	std::vector<unsigned char> result;
	REX(TRN_PDFDocSaveViewerOptimizedBuffer(mp_doc, (const char**)&buf, &size, opts.m_obj));
	result.resize(size);
	memcpy(&result[0], buf, size);
	return result;
}
...
inline void PDFDoc::MergeXFDF(Filters::Filter stream, const MergeXFDFOptions* opts)
{
	OptToOptionbase(opts, MergeXFDFOptions);
	REX(TRN_PDFDocMergeXFDF(mp_doc, stream.m_impl, &opt_in));
}

inline void PDFDoc::MergeXFDF(const UString& xfdf, const MergeXFDFOptions* opts)
{
	OptToOptionbase(opts, MergeXFDFOptions);
	REX(TRN_PDFDocMergeXFDFString(mp_doc, xfdf.mp_impl, &opt_in));
}
...
inline SDF::Obj Config::GetRBGroups() const {
	TRN_Obj result;
	REX(TRN_OCGConfigGetRBGroups(mp_obj, &result));
	return SDF::Obj(result);
}

inline void Config::SetRBGroups(SDF::Obj rbgroups_array) {
	REX(TRN_OCGConfigSetRBGroups(mp_obj, rbgroups_array.mp_obj));
}
