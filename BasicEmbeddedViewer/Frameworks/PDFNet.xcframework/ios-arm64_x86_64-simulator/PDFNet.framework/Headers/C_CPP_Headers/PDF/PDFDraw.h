...
public:
	BitmapInfo(int w, int h, int s, double d, char* b) {
		...
		dpi = d;
		if (h * s > 0) {
			buf.resize((std::vector<unsigned char>::size_type)(h * s));
			if (b)
			{
				memcpy(&buf[0], b, (size_t)(h * s));
			}
		}
	}
	int width;
...
public: 
	...
	* @encoder format - The output image format. See the overloaded method for details.
