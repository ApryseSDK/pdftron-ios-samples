...
public:
		...
		const RectCollection& rects, int index)
	{
		...
		SDF::Obj arr = GetArray(dict, key);
		while(arr.Size() <= (size_t)index)
		{
			arr.PushBackArray();
		}
		SDF::Obj innerArray = arr.GetAt((size_t)index);
	...
	static double ColorPtToNumber(const ColorPt& cp) 
	{
		...
		UInt32 alpha = 255;
		if(num_comp == 1) {
			red = (UInt32)(255*cp.Get(0));
			green = red;
			blue = red;
		}
		else if(num_comp == 3 || num_comp == 4) {
			red = (UInt32)(255*cp.Get(0));
			green = (UInt32)(255*cp.Get(1));
			blue = (UInt32)(255*cp.Get(2));
		}
		if(num_comp == 4) {
			alpha = (UInt32)(255*cp.Get(3));
		}
		const UInt32 num = ((0xFF&alpha) << 24)
						| ((0xFF&red) << 16)
						| ((0xFF&green) << 8)
						| ((0xFF&blue) << 0);
		return (double)num;
