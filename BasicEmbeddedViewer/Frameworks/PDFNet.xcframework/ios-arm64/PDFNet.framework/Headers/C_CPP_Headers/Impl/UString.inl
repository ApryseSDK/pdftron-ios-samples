...
{
	std::string s;
	const int sz = this->ConvertToAscii(0, 0, false);
	if ( sz > 0 ) 
	{ 
		s.resize((std::string::size_type)sz);
...
inline std::string UString::ConvertToUtf8() const {
	std::string s;
	const int sz = this->ConvertToUtf8(0, 0, false);
	if ( sz > 0 ) 
	{ 
		s.resize((std::string::size_type)sz);
...
inline int UString::ConvertToUtf32( UInt32* in_out_buf, int buf_sz, bool null_term ) const
{
	int result;
	REX(TRN_UStringConvertToUtf32(mp_impl,in_out_buf,buf_sz,null_term,&result));
	return result;
}
...
{
	const int sz = this->ConvertToUtf32(0, 0, false);
	std::basic_string<UInt32> s;
	if ( sz > 0 ) 
	{ 
		s.resize((std::basic_string<UInt32>::size_type)sz);
