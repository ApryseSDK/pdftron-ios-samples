...
inline void PDFViewCtrl::SetRightToLeftLanguage(bool flag)
{
	REX(TRN_PDFViewCtrlSetRightToLeftLanguage(mp_view, BToTB(flag)));
}

inline bool PDFViewCtrl::GetRightToLeftLanguage() const
{
	TRN_Bool result = 0;
	REX(TRN_PDFViewCtrlGetRightToLeftLanguage(mp_view, &result));
	return TBToB(result);
}
...
inline 	void  PDFViewCtrl::SaveCurrentConfiguration( const UString & path, Common::ProgressMonitor*) 
{
...
inline void PDFViewCtrl::UpdateCustomHeaders(const class HTTPRequestOptions& options)
{
	REX(TRN_PDFViewCtrlUpdateCustomHeaders(mp_view, options.m_obj));
}
