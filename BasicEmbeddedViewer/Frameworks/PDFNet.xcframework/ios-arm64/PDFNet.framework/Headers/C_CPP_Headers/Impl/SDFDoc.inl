...
inline bool DictIterator::HasCurrent() {
	RetBool(TRN_DictIteratorHasCurrent(mp_impl,&result));
}
...
#endif
inline void Obj::SetStreamData( const std::vector<unsigned char>& data, 
				   Filters::Filter filter_chain )
{
	filter_chain.m_owner = false;
	REX(TRN_ObjSetStreamDataWithFilter( mp_obj,  data.empty() ? 0 : (const char*)&data[0], data.size(), filter_chain.m_impl ));
	// Note: Transfer the ownership
	filter_chain.m_impl = 0;
}
...
#ifndef SWIG
inline void SDFDoc::Save(const UString& path, UInt32 flags, Common::ProgressMonitor*, const char* header) {
	REX(TRN_SDFDocSave(mp_doc,path.mp_impl,flags,0,header));
}
#endif
...
#ifndef SWIG
inline void SDFDoc::Save(const char* &out_buf, size_t& out_buf_size, UInt32 flags, Common::ProgressMonitor*, const char* header) {
